...
(function () {
  ...
  // URL pública de la web (GitHub Pages). Se usa para que el enlace del email
  // de confirmación apunte SIEMPRE aquí y no a localhost, aunque estés probando
  // en local. Si algún día usas dominio propio, cámbialo aquí.
  var SITE_URL = "https://sergimartin2003-cmd.github.io/Influjob/";
  ...
  function boot() {
    ...
    modal.innerHTML =
      '<div class="auth-inner">' +
        ...
        '<form class="apply-form auth-view" id="auth-view-signup" hidden novalidate>' +
          ...
          '<div class="form-group"><label for="auth-su-nif">CIF / NIF de la empresa <span class="auth-optional">(opcional)</span></label>' +
            '<input type="text" id="auth-su-nif" placeholder="B12345678" autocomplete="off" maxlength="9">' +
            '<span class="auth-field-hint" id="auth-nif-hint">Si lo indicas, verificamos tu empresa y le damos un sello de confianza.</span></div>' +
        ...
        '<div class="auth-view" id="auth-view-account" hidden>' +
          ...
          '</dl>' +
          '<a href="perfil.html" class="btn btn-primary auth-submit" id="auth-panel-link">Ir a mi panel</a>' +
          '<a href="publicar-oferta.html" class="btn btn-outline-blue auth-submit" id="auth-publish-link">Publicar una oferta</a>' +
        ...
        if (session && company) {
          ...
          if (badge) {
            ...
            badge.textContent = ok
              ...
              : "Empresa sin verificar · la verificación fiscal es opcional";
          }
          // La verificación es opcional: siempre se puede publicar
          if (pubLink) {
            pubLink.setAttribute("aria-disabled", "false");
            pubLink.classList.remove("is-disabled");
          }
        } else if (session) {
    ...
    // Los enlaces a "Publicar oferta" exigen sesión: si no la hay, abrimos el
    // modal de acceso en vez de llevar a la página (que también está protegida).
    document.addEventListener("click", function (e) {
      var link = e.target.closest('a[href$="publicar-oferta.html"]');
    ...
    $("#auth-view-signup", modal).addEventListener("submit", function (e) {
      ...
      // Verificación fiscal OPCIONAL: si lo rellenan, debe ser válido; si lo
      // dejan vacío, la empresa se crea igual (sin sello de verificación).
      var nif = $("#auth-su-nif", modal).value.trim().toUpperCase();
      if (nif && !validarNifCif(nif)) {
        showError("El CIF/NIF no es válido. Déjalo vacío o pon uno correcto (p. ej. B12345674).");
